...
use super::{StatementResult, ResponseResult};
...
impl From<crate::types::ResponseError> for crate::influxql::response::ResponseError {
    fn from(error: crate::types::ResponseError) -> Self {
        match error {
            crate::types::ResponseError::InfluxqlResponseError(error) => error,
            _ => panic!("Wrong type of response error"),
        }
    }
}
...
/// # use std::collections::HashMap;
/// # use chrono::{DateTime, Utc};
/// # use rinfluxdb::influxql::{from_str, ResponseError};
/// # use rinfluxdb::types::Value;
///
/// use std::convert::{TryFrom, TryInto};
///
    ...
    E: Into<ResponseError>,
{
    ...
    for row in series.values {
        let instant = row[0].as_str().ok_or_else(|| ResponseError::ValueError("index is not encoded as string".into()))?;
