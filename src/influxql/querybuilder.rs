...
impl QueryBuilder {
    ...
    pub fn build(self) -> Query {
        ...
        match (self.database, self.retention_policy) {
            (Some(database), Some(retention_policy)) => write!(&mut result, " FROM {}.{}.{}", database, retention_policy, self.measurement).unwrap(),
            (Some(database), None) => write!(&mut result, " FROM {}..{}", database, self.measurement).unwrap(),
            (None, Some(retention_policy)) => write!(&mut result, " FROM .{}.{}", retention_policy, self.measurement).unwrap(),
        ...
        if self.start.is_some() || self.stop.is_some() {
            ...
            match (self.start, self.stop) {
                (Some(start), Some(stop)) => write!(&mut result, " time > '{}' AND time < '{}'", start.to_rfc3339_opts(SecondsFormat::AutoSi, true), stop.to_rfc3339_opts(SecondsFormat::AutoSi, true)).unwrap(),
                (Some(start), None) => write!(&mut result, " time > '{}'", start.to_rfc3339_opts(SecondsFormat::AutoSi, true)).unwrap(),
                (None, Some(stop)) => write!(&mut result, " time < '{}'", stop.to_rfc3339_opts(SecondsFormat::AutoSi, true)).unwrap(),
