...
use crate::influxql::ResponseError as InfluxqlResponseError;
use crate::flux::ResponseError as FluxResponseError;
...
impl ToString for InstantOrDuration {
    fn to_string(&self) -> String {
        match self {
            InstantOrDuration::Instant(instant) => format!("'{}'", instant.to_rfc3339_opts(SecondsFormat::AutoSi, true)),
...
pub enum ResponseError {
    /// Error occurred while parsing a response to an InfluxQL query
    #[error("InfluxQL response error")]
    InfluxqlResponseError(#[from] InfluxqlResponseError),

    /// Error occurred while parsing a response to an Flux query
    #[error("Flux response error")]
    FluxResponseError(#[from] FluxResponseError),
}
